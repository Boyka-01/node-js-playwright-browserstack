...
test('Complete Form Submission Flow', async ({ page }) => {
    ...
    await saveScreenshot(page, 'form-submitted');

    try {
        const successMessageLocator = page.locator(
            '//div[@id="gform_confirmation_message_2" and @class="gform_confirmation_message_2 gform_confirmation_message"]'
        );
        ...
        await saveScreenshot(page, 'response-failed');
    }
});
